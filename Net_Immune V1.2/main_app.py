...
def analyze_threat(prompt_type, text_to_analyze):
    ...
    elif prompt_type == "file": sys_prompt = "Analyze this downloaded file name. Look for malware, double extensions, pirated software, game repacks, or .torrent files. If it relates to piracy, torrents, or hacking, categorize exactly as [UNSAFE_PIRACY]. If it is a virus, [MALWARE]. Otherwise, [SAFE]. Format exactly: [TAG] Reason."
    elif prompt_type == "manual_file": sys_prompt = "Analyze this manually uploaded file name. Look for malicious intent, double extensions, or script types. Categorize exactly: [SAFE], [MALWARE], [UNSAFE_PIRACY]. Format exactly: [TAG] Reason."
...
class SetupWizard:
    def __init__(self):
        ...
        try:
            icon_img = tk.PhotoImage(file=resource_path("mascot.ico"))
    ...
    def manual_file_scan(self):
        ...
        if file_path:
            ...
            def run_scan():
                ...
                res = analyze_threat("manual_file", filename)
...
class FloatingMascot:
    def __init__(self, root):
        ...
        # SUPPRESS=TRUE ensures pressing the key doesn't actually type a backtick over highlighted text!
    ...
    def trigger_caps_scan(self, event=None):
        ...
        if getattr(self, 'is_hotkey_scanning', False): return # COOLDOWN LOCK: Stops Spam!
        
        self.is_hotkey_scanning = True # LOCK BACKGROUND LOOP
        ...
        if len(text) > 2 and text != self.previous_clipboard:
            ...
            self.previous_clipboard = text # Tell background loop we already saw this
            
        elif text == self.previous_clipboard:
            self.log_to_dashboard("> [Hotkey] Text unchanged. Skipping duplicate scan.")
            
        self.root.after(1500, lambda: setattr(self, 'is_hotkey_scanning', False)) # UNLOCK after 1.5 sec
    ...
    def ai_background_loop(self):
        ...
        while self.running:
            ...
            if self.agent1_on and not getattr(self, 'is_hotkey_scanning', False): # ONLY RUN IF HOTKEY IS IDLE
                try:
            ...
            if self.agent3_on:
                try:
                    ...
                    new_files = [f for f in current_downloads if f not in known_downloads and not f.endswith(".tmp") and not f.endswith(".crdownload")]
                    for new_file in new_files:
                        self.log_to_dashboard(f"\n[Downloads] Scanning new file: {new_file}")
                        res = analyze_threat("file", new_file)
                        clean_msg = res.split("] ")[-1] if "] " in res else res
                        ...
                        if "[ERROR]" in res.upper():
                            ...
                            self.root.after(0, self.trigger_alert_emotion)
                            send_alert("🚨 Malicious Download Blocked!", clean_msg)
                            self.log_to_dashboard(res)
                        else:
            ...
            if self.agent4_on and gw:
                ...
                if web_timer >= 60:
                    try:
                        ...
                        if active_title and active_title != last_window_title:
                            lower_title = active_title.lower()
                            browsers = ["chrome", "brave", "edge", "firefox", "opera"]
                            
                            if any(browser in lower_title for browser in browsers):
                                ...
                                if "[ERROR]" in res.upper():
                                    ...
                                    self.root.after(0, self.trigger_alert_emotion)
                                    send_alert("⚠️ Unsafe Web Environment!", clean_msg)
                                    self.log_to_dashboard(res)
                                else:
...
def start_main_app():
    ...
    root = tk.Tk()
    try: root.iconbitmap(resource_path("mascot.ico"))
    except: pass
